...
export class Path {
  ...
  isModule() {
    return minimatch(this.filename, patterns.module(this.options))
  }
  isModuleIndex() {
    return minimatch(this.filename, patterns.moduleIndex(this.options, this.ext))
  }
  isModuleUtil() {
    return minimatch(this.filename, patterns.moduleUtil(this.options, this.ext))
  }
  isRoot() {
    return minimatch(this.filename, patterns.root(this.options))
  }
  // isSrc() {
  //   return minimatch(this.filename, patterns.src(this.options))
  // }
  isUtils() {
    return minimatch(this.filename, patterns.utils(this.options))
  }
  isJson() {
    return minimatch(this.filename, patterns.json(this.options), {nocase: true})
  }
  isTest() {
    return minimatch(this.filename, patterns.test(this.options))
  }
  isTestModule() {
    return minimatch(this.filename, patterns.testModule(this.options))
  }
  isTestRoot() {
    return minimatch(this.filename, patterns.testRoot(this.options))
  }
  isTestSpecial() {
    return minimatch(this.filename, patterns.testSpecial(this.options))
  }
  isModuleUtilOrUtils() {
